...
The remote sources are polite by design: robots.txt Disallow rules are always honored (a disallow-all robots skips the sitemap and crawl entirely), the sitemap set is capped at 30 pages preferring shallow paths, and the crawl fetches one page at a time with a 500ms pause, identifies itself as `qa-core-agent-discovery`, stays same-origin, and stops at depth 2 or 15 pages. A large sitemap set is narrowed by a cheap Haiku call to the most relevant page per feature (max 8), with a deterministic shallowest-first fallback.
