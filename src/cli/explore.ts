...
function parseArgs(argv: string[]): ParsedArgs {
  ...
  const parsed: ParsedArgs = { lang: 'ts', review: false, pom: true };
  ...
  if (!parsed.url && !parsed.fromPlan) {
    console.error('Usage:');
    console.error('  npm run explore -- <url> [--lang ts|js] [--name foo] [--out dir] [--review] [--no-pom]');
  ...
  const result = await explore({
    ...
    onEvent: (e) => {
      switch (e.type) {
        ...
        case 'done':
          console.log(`\n✓ ${e.scenarios} scenario(s) recorded`); break;
      }
    },
  });
